...
def UIimageTraining_LoadEvents(parent, Config):
    print "UIimageProcessing_LoadEvents"
...
#    def predict(self, T):
#        return [self.model.predict(np.asarray(T[i])) for i in range(0, len(T))]
...
def TrainModel():
    ...
    if( (Rb_FisherRec or Rb_LBPHRec or Rb_EigenRec) and Hs_TrainTestSize > 0  ):
        ...
        for i in range(0, maxItems):
            [Y_pred_temp, Y_confidence_temp] = model.predict(np.asarray(X_test[i]))
